"""
Base Video Decoder and Video Metadata Probe Utilities.
Handles metadata extraction (via ffprobe/OpenCV fallback) and
SIMD-accelerated NV12 full-range to BGR conversion.
"""
import os
import json
import subprocess
...
def nv12_to_bgr_full_range(
    ...
    restoring full dynamic range (0-255) with 2.1 ms latency.
    """
    if h is None:
        h = nv12.shape[0] * 2 // 3
    if w is None:
...
class BaseVideoDecoder(ABC):
    """Abstract Base Class for video decoders in the ALPR pipeline."""
